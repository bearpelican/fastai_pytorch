...
def parse_args(elt, ignore_first=False) -> str:
    ...
    return f'({parsedargs}){parsedreturn}'


def format_docstring(elt, arg_comments={}):
    parsed = ""
    doc = parse_docstring(inspect.getdoc(elt))
    description = doc['long_description'] or doc['short_description']
    if description: parsed += f'\n\n{description}'

    resolved_comments = {**doc.get('params', {}), **arg_comments} # arg_comments takes priority
    args = inspect.getfullargspec(elt)[0]
...
def get_ft_doc(elt, full_name):
    doc = f'**{full_name}**' + parse_args(elt, False)
    return doc
...
def get_cls_doc(elt, full_name):
    ...
    doc = f'**{full_name}**'
    doc += parse_args(elt, ignore_first=True)
    if parent_class != object: doc += f' ~subclass of **{format_type(parent_class)}**~'
    return doc
...
def show_doc(elt, doc_string=True, full_name=None, arg_comments={}, alt_doc_string=''):
    ...
    if doc_string and (inspect.getdoc(elt) or arg_comments): doc += '\n' + format_docstring(elt, arg_comments)
